...
gAddUp() {
  ...
  echo -e "Adding upstream remote repository for $repo_name! 🌍\n\n"
  git remote add upstream "https://github.com/$username/$repo_name.git"
} # Adds an upstream remote repo. Usage: gAddUpstream <repo-name> <username>. Usually used when working on a forked repo and upstream is the czar or original repo.

gFetchUp() {
  echo -e "Fetching latest changes from upstream remote repository! 🌍\n\n"
  git fetch upstream
} # Fetches latest changes from upstream remote repo.

gMerge() {
  local branch=${1:-main}
  echo -e "Merging latest changes from upstream/$branch into local $branch branch! 🌍\n\n"
  git merge "upstream/$branch"
} # Merges latest changes from upstream remote repo into local branch (default: main). Usage: gMerge [branch]
...
gPush() {
  ...
  echo -e "Alright, we done. Push it up to $remote/$branch! 🫷⬆️\n\n"
  git push "$remote" "$branch"
} # Pushes local changes to remote (default: origin) and branch (default: main). Usage: gPush [remote] [branch].
...
# CONVENTIONAL COMMITS - INTERACTIVE MESSAGE CRAFTER (no auto-commit)
# ----------------------------------------------
# Usage:
#   gcm -> Guided flow to build a Conventional Commit message
gCommitMessage() {
  ...
  git diff --stat || true
  echo "***************************************************"
  read -r "_cont?Continue with message crafting? (Y/n): "
  if [[ "${_cont:-Y}" =~ ^[Nn]$ ]]; then
    echo "Cancelled."
    return 1
  fi
  ...
  # Commit type menu
  local types=(feat fix docs style refactor test chore)
  ...
  local type_idx type
  while true; do
    read -r "type_idx?Enter number (1-${#types[@]}): "
    if [[ "$type_idx" =~ ^[1-9][0-9]*$ && $type_idx -ge 1 && $type_idx -le ${#types[@]} ]]; then
      type=${types[$((type_idx-1))]}
      break
    fi
    echo "Please enter a valid number."
  done

  # Suggest scope from changed files (top-level segments)
  local files scopes raw_files
  raw_files=$(git diff --name-only HEAD 2>/dev/null || true)
  if [[ -z "$raw_files" ]]; then
    raw_files=$(git ls-files -m -o --exclude-standard || true)
  fi
  if [[ -n "$raw_files" ]]; then
    scopes=($(echo "$raw_files" | sed 's#^\./##' | awk -F'/' '{print $1}' | sort -u | sed '/^$/d'))
  else
    scopes=()
  fi
  ...
  local scope=""
  if (( ${#scopes[@]} > 0 )); then
    echo "Select scope (suggested from changes):"
    local j=1
    for s in "${scopes[@]}"; do
      echo "  $j) $s"
      j=$((j+1))
    ...
    echo "  c) custom"
    while true; do
      read -r "ans?Enter selection: "
      if [[ "$ans" == "0" ]]; then
        scope=""
        break
      elif [[ "$ans" == "c" || "$ans" == "C" ]]; then
        read -r "scope?Enter custom scope (kebab-case recommended): "
        break
      elif [[ "$ans" =~ ^[1-9][0-9]*$ && $ans -ge 1 && $ans -le ${#scopes[@]} ]]; then
        scope=${scopes[$((ans-1))]}
        break
      else
        echo "Please choose 0, c, or a valid number."
      fi
    done
  else
    read -r "scope?Enter scope (optional, empty for none): "
  fi

  # Breaking change flag
  local bang=""
  read -r "brk?Is this a breaking change? (y/N): "
  if [[ "${brk:-N}" =~ ^[Yy]$ ]]; then bang="!"; fi

  # Subject (<= 50 chars), imperative, no trailing period
  local subject
  while true; do
    read -r "subject?Short description (<= 50 chars, no period): "
    subject=${subject%%.}
    subject=$(echo "$subject" | sed -E 's/^\s+|\s+$//g')
    if [[ -n "$subject" && ${#subject} -le 50 ]]; then break; fi
    echo "Subject required and must be 50 chars or fewer."
  done

  # Optional body (multi-line until empty line)
  echo "Enter body (optional). Finish by submitting an empty line:"
  local body line
  body=""
  ...
  local footer=""
  read -r "footer?Footer (optional, e.g., Closes #123): "

  # Optional BREAKING CHANGE details
  local breaking_detail=""
  if [[ -n "$bang" ]]; then
    read -r "breaking_detail?BREAKING CHANGE details (required for !): "
    if [[ -z "$breaking_detail" ]]; then
      echo "Details recommended for breaking changes."
    fi
  fi

  # Build header and full message
  local header
  if [[ -n "$scope" ]]; then
    header="$type$bang($scope): $subject"
  else
    header="$type$bang: $subject"
  fi

  # Validate header against Conventional Commits
  local re='^(feat|fix|docs|style|refactor|test|chore)(!?)((\([a-zA-Z0-9_.\-\/]+\))?)\: .{1,50}$'
  if ! echo "$header" | grep -Eq "$re"; then
    echo "❌ Header doesn't meet Conventional Commits format."
    echo "   $header"
    return 1
  fi
  ...
  if [[ -n "$bang" ]]; then
    msg+="\n\nBREAKING CHANGE: ${breaking_detail:-Details omitted}"
  fi

  echo "***************************************************"
  echo "Preview:"
  echo "---------------------------------------------------"
  echo "Your draft:"; echo "$msg"
  echo "***************************************************"

  # Copy to clipboard and print to stdout
  if command -v pbcopy >/dev/null 2>&1; then
    printf "%s" "$msg" | pbcopy && echo "✅ Copied to clipboard."
  fi

  # Save to a temp file for convenience
  local tmpfile
  tmpfile=$(mktemp -t commitmsg.XXXXXX)
  printf "%s\n" "$msg" > "$tmpfile"
  echo "Saved at: $tmpfile"

  echo "---------------------------------------------------"
  echo "Paste into your editor when gCommit opens it."
  echo "Header: $header"
  return 0
}

alias gcm="gCommitMessage"
